...
- count of samples with prob < 0.1

This mirrors scripts/eval_forget_lora.py but loads a full model directly.

Usage 1: Single model
CUDA_VISIBLE_DEVICES=0 python scripts/eval_full_fused.py \
  --model \
    "/work/zhb/CTCC-main/outputs/mistral_merges/task/Mistral-7B-Instruct-v0.3/0.5-0.5" \
  --dataset \
    "/work/zhb/open-unlearning-main_1/forget10.json" \
  --batch-size 16 \
  --max-length 512 \
  --gpu-ids 0 \
  --auto-out true  # 自动把结果保存到 /work/zhb/CTCC-main/fsr/task/0.5_0.5_prob_rouge.jsonl

Usage 2: 批量评估多个策略与比例
CUDA_VISIBLE_DEVICES=0 python scripts/eval_full_fused.py \
  --batch true \
  --scan-root "/work/zhb/CTCC-main/outputs/mistral_merges" \
  --strategies "task,ties,dare_task,dare_ties" \
  --model-name "Mistral-7B-Instruct-v0.3" \
  --out-base "/work/zhb/CTCC-main/fsr" \
  --dataset "/work/zhb/open-unlearning-main_1/forget10.json" \
  --batch-size 16 --max-length 512 --gpu-ids 0
"""
import os
import sys
import json
...
def load_full_model(model_path: str, dtype: torch.dtype, gpu_ids: list):
    # device_map setup
    device_map = "auto"
...
def get_template_args_for_model(tokenizer):
    ...
    if "mistral" in name:
        ...
        # We avoid explicitly injecting BOS/EOS here, relying on tokenizer(add_special_tokens=True)
        return dict(
...
def _discover_ratio_dirs(strategy_dir: Path) -> list:
    ...
    If none are found but the directory itself looks like a model, return [strategy_dir.name]
    """
...
def evaluate_and_save(
    ...
    prob_lt_01 = int(np.sum(np.array(probs) < 0.1)) if len(probs) else 0
...
def main():
    ...
    parser.add_argument("--out", required=False, default="/work/zhb/open-unlearning-main_1/saves/eval/fused_forget10_prob_rouge.jsonl")
    ...
    parser.add_argument("--scan-root", type=str, default="/work/zhb/CTCC-main/outputs/mistral_merges", help="Root under which to scan models")
    ...
    parser.add_argument("--out-base", type=str, default="/work/zhb/CTCC-main/fsr", help="Base directory to save jsonl results")
