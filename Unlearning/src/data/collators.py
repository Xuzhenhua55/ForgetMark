...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # Handle case where instances are not dictionaries (e.g., strings for tokenization)
        if not isinstance(instances[0], dict):
            # Only tokenize if all instances are strings
            if all(isinstance(inst, str) for inst in instances):
                return self.tokenizer(instances, **self.tokenizer_args)
            else:
                # For non-string, non-dict data, assume it's already processed
                # This shouldn't happen in normal flow, but handle gracefully
                raise ValueError(f"Unexpected data type in collator: {type(instances[0])}")
