...
def load_trainer(
    ...
    # Extract TrainingArguments and method-specific args (e.g., for GradDiff)
    trainer_args_cfg = dict(trainer_cfg.args)
    method_args = dict(trainer_cfg.get("method_args", {}))

    # Pop custom (non-Transformers) arguments from trainer.args
    # to avoid TypeError when instantiating TrainingArguments
    for custom_key in ["gamma", "alpha", "retain_loss_type"]:
        if custom_key in trainer_args_cfg:
            method_args[custom_key] = trainer_args_cfg.pop(custom_key)

    # Build transformers.TrainingArguments with the cleaned args
    trainer_args = load_trainer_args(trainer_args_cfg, train_dataset)
